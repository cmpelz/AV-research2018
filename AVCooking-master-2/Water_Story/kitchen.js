  /*FILE SETTING UP INTERACTIVE KITCHEN SIMULATION, FOLLOWS COOKING "STORY" TO ENCOURAGE PARTICIPANTS TO INTERACT WITH SIMULATION*/

/*boolean and method to freeze visualization*/
var run = true;
function freeze(){
  run = false;
}
...
/*object describing the kitchen as a set of appliances, each with a corresponding color*/
function KitchenSim() {
    this.sim_appliances = [new Appliance("fridge", "#70db70"), new Appliance("light", "#cc0000"), new Appliance("rangehood", "#ffcc00"), new Appliance("oven", "#4dc3ff"), new Appliance("toaster", "#cc99ff")],
...
/*function insertImg( imgSrc, divID, width, height ){
    ...
    var mapShape1 = '<area alt="" title="" href="javascript:changePic( 0 ), updateScoreClick()" shape="poly" coords="48,310,45,293,44,280,42,265,40,247,40,231,40,212,40,198,41,178,44,159,46,140,50,127,57,122,68,121,84,118,98,117,116,119,130,120,141,121,154,124,164,126,169,133,167,145,166,157,165,169,163,183,162,197,160,214,159,229,157,242,156,255,155,266,153,282,152,294,151,302,150,308" />';
    var mapShape2 = '<area alt="" title="" href="javascript:changePic( 1 ), updateScoreClick()" shape="poly" coords="200,1,200,37,193,38,197,44,171,59,178,64,185,64,195,65,214,65,230,63,233,58,205,42,209,37,203,37" />';
    var mapShape3 = '<area alt="" title="" href="javascript:changePic( 2 ), updateScoreClick()"shape="poly" coords="420,137,413,146,414,159,533,163,533,158,528,156,521,144,501,137" />';
    var mapShape4 = '<area alt="" title="" href="javascript:changePic( 3 ), updateScoreClick()" shape="poly" coords="420,210,415,212,412,218,412,230,423,230,420,305,535,305,532,229,522,228,521,216,515,210" />';
    var mapShape5 = '<area alt="" title="" href="javascript:changePic( 4 ), updateScoreClick()" shape="poly" coords="539,199,543,226,540,229,586,229,585,225,589,207,586,201,580,199,547,196" />';
*/
    /*
...
/*functions used in cooking simulation to display graphics of cooking meal and instructions for the next action*/
...
function lookingInTheFridge(){
  ...
  document.getElementById("instructions").innerHTML = "Now turn on the stove to cook your eggs"
  frdOn = false;
}

/*
function hoodOff(){
    document.getElementById("instructions").innerHTML = "The eggs are done!"
    tstOn=true;

}*/

function stoveOn(){
    document.getElementById("instructions").innerHTML = "Now turn on the rangehood to cook your eggs"
    ovnOn = false;
}
...
/*if participant presses the wrong appliance in response to the instructions displayed, error message is displayed*/
function wrongAction(n){
  wrongPic(n);
}

/*clears error message after incorrect action and corrects mistake*/
function clearWrong(){
  document.getElementById("wrong").innerHTML = "";
}


/*function to change kithen image depending on what appliance was cliked, so the correct appliances are lit up*/
function changePic( num ){
    /*file path variables to compare what image was displayed before and load new image*/
    var filePath1 = "http://sysnet.cs.williams.edu/~20gem3/Water_Story/labeled-kitchen/";
    ...
    if ( num == 0 && srcImg == ( filePath1 + "kitchen_2_all_off.jpg" ) ) {
        ...
        document.getElementById("x").src = filePath2 + "kitchen_2_hood_toaster_on.jpg";
	      kitchen.switchState(sim_appliances[0]);                                                                                                                                                 wrongAction(num);
	...
	kitchen.switchState(sim_appliances[1]);
        ...
        document.getElementById("x").src = filePath2 + "kitchen_2_fridge_light_hood_on.jpg";
	kitchen.switchState(sim_appliances[1]);                                                                                                                                                 wrongAction(num);
	...
	wrongAction(num);
      ...
      wrongAction(num);
        ...
        document.getElementById("x").src = filePath2 + "kitchen_2_fridge_on.jpg";
	kitchen.switchState(sim_appliances[2]);                                                                                                                                                 wrongAction(num);
	...
	kitchen.switchState(sim_appliances[2]);                                                                                                                              wrongAction(num);
        ...
        if(ovnOn){
          ...
          setTimeout(waitingOnEggs,2000);
	         kitchen.switchState(sim_appliances[2]);
	        //setTimeout(hoodOff, 5000);
        }
	...
	wrongAction(num);
        ...
        document.getElementById("x").src = filePath2 + "kitchen_2_fridge_hood_on.jpg";
	kitchen.switchState(sim_appliances[3]);                                                                                                                                                 wrongAction(num);
